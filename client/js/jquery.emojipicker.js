  ...
  $.extend(EmojiPicker.prototype, {
    ...
    addPickerIcon: function () {
      ...
      if (this.settings.button) {
        ...
        var iconHeight = elementHeight > MAX_ICON_HEIGHT ? MAX_ICON_HEIGHT : elementHeight;
        ...
        this.$wrapper = this.$el.wrap("<div class='emojiPickerIconWrap'></div>").parent();
    ...
    createPicker: function () {
      // Show template
      this.$picker = $(getPickerHTML()).appendTo(this.$container).width(this.settings.width).height(this.settings.height).css('z-index', 10000);

      // Picker height
      this.$picker.find('.sections').height(parseInt(this.settings.height) - 40); // 40 is height of the tabs
    ...
    listen: function () {
      ...
      if (this.settings.button) {
        ...
        this.$wrapper.find('.emojiPickerIcon').click($.proxy(this.iconClicked, this));
      }
      ...
      this.$picker.find('.sections').scroll($.proxy(this.emojiScroll, this));

      this.$picker.click($.proxy(this.pickerClicked, this));

      // Key events for search
      this.$picker.find('section.search input').on('keyup search', $.proxy(this.searchCharEntered, this));
    ...
    emojiMouseover: function (e) {
      ...
      $shortcode
        .find('.info')
        .show()
        .html('<div class="emoji emoji-' + emojiShortcode + '"></div><em>' + emojiShortcode + '</em>');
    },
    ...
    emojiCategoryClicked: function (e) {
      ...
      var scrollDistance = heightOfSectionsHidden + heightOfSectionToPageTop - heightOfSectionsToPageTop;
    ...
    emojiScroll: function (e) {
      ...
      $.each(sections, function (key, value) {
        ...
        if (offsetFromTop <= 0) {
          ...
          $(section)
            .parents('.emojiPicker')
            .find('nav .tab[data-tab=' + section.className + ']')
            .addClass('active');
        }
      });
    },
